...
class LinearSatellite(CtrlAffSys):
    ...
    def get_mask(self, x: Tensor, type: str = 'safe') -> Tensor:
        ...
        if   type == 'safe'  : return torch.logical_and(distance <= 0.75, distance >= 0.25)
        elif type == 'unsafe': return torch.logical_or(distance <= 0.25, distance >= 0.75)
        elif type == 'goal'  : return torch.ones(x.shape[0], dtype = torch.bool)
