...
if __name__ == '__main__':
    wandb.init(
        project = 'nn_barrier', 
        entity = 'hongjue', 
        name = 'linsate_deriv_demo'
    )
    init_seed(1)
    
    ckpt_pth = './outputs/linsate_deriv_demo'
    ...
    for epoch in range(300):
        train_loss, train_safe_loss, train_unsafe_loss, train_deriv_loss = 0, 0, 0, 0
        for batch_idx, batch in enumerate(train_iter):
            ...
            finish = time.time()
            print(f'Epoch {epoch:3} | batch_idx {batch_idx:2}, Train Loss: {loss.item():.3e}, Safe Loss: {safe_loss.item():.3e}, Unsafe Loss: {unsafe_loss.item():.3e}, Relaxation Loss: {deriv_loss.item():.3e}, Time: {finish - start:.3f}')
            train_loss += loss.item()
            train_safe_loss += safe_loss.item()
            train_unsafe_loss += unsafe_loss.item()
            train_deriv_loss += deriv_loss.item()        
            
        val_loss, val_safe_loss, val_unsafe_loss, val_deriv_loss = 0, 0, 0, 0
        with torch.no_grad():
        ...
        print('-' * 150)
        print(f'Epoch {epoch:3} | Train Loss: {train_loss / len(train_iter):.3e}, Safe Loss: {train_safe_loss / len(train_iter):.3e}, Unsafe Loss: {train_unsafe_loss / len(train_iter):.3e}, Relaxation Loss: {train_deriv_loss / len(train_iter):.3e}')
        print(f'Epoch {epoch:3} | Val Loss:   {val_loss / len(val_iter):.3e}, Safe Loss: {val_safe_loss / len(val_iter):.3e}, Unsafe Loss: {val_unsafe_loss / len(val_iter):.3e}, Relaxation Loss: {val_deriv_loss / len(val_iter):.3e}')
        wandb.log({
            'epoch': epoch,
            'train': {
